<?php
// TODO: improve this.
// get_http_transport: use static vars
// get_remote_content: return array( content, status, code )
...
function get_http_transport( $url ) {
...
function get_remote_content( $url,  $maxlen = 4096, $timeout = 5 ) {
	$url = sanitize_url( $url );

	$transport = get_http_transport( $url );
	if( $transport ) {
		$content = call_user_func( 'get_remote_content_'.$transport, $url, $maxlen, $timeout );
	} else {
		$content = false;
	}
...
function get_remote_content_curl( $url, $maxlen = 4096, $timeout = 5 ) {
...
function get_remote_content_fopen( $url, $maxlen = 4096, $timeout = 5 ) {
	$content = false;
	
	$initial_timeout = @ini_set( 'default_socket_timeout', $timeout );
	$initial_user_agent = @ini_set( 'user_agent', http_user_agent( ) );
...
function get_remote_content_fsockopen( $url, $maxlen = 4096, $timeout = 5 ) {
...
function http_user_agent( ) {
	return  'QRMANAGER ' . QRGEN_VERSION . ' (running on' . QRGEN_SITE . ' )' ;
}
